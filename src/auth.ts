...
export class AuthClient {
	...
	async linkWalletForAuth(
		...
		return this.rpc.call<OnboardSession>("tenzro_linkWalletForAuth", params);
	}
	 ...
	 *
	 * `didEnvelope` is the hex envelope proving control of the token's bearer
	 * DID **or** of the controller DID that authorized it, bound to method
	 * `tenzro_revokeJwt` with the `jti` bytes as the params hash. A `jti`
	 * travels in every audit row, so it is an identifier, not a credential.
	 */
	async revokeJwt(
		jti: string,
		didEnvelope: string,
	 ...
	 * transitively, and the TDIP revocation is gossiped to peer registries.
	 *
	 * Requires **both** the operator admin token on the transport and a
	 * `didEnvelope` from the DID itself or, for a machine, its controller —
	...
	async forgetIdentity(
		did: string,
	): Promise<{ did: string; status: string; note: string }> {
	 ...
	 */
	async listPendingApprovals(approverDid: string): Promise<PendingApprovals> {
