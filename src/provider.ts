...
/**
 * Who may call a served model.
 *
 * A property of the model, not of the node — a node whose service-key gate is
 * on can still publish models to the network.
 *
 * - `network` (default) — announced on `tenzro/models` and reachable by **any
 *   caller who pays**, with no service key and no prior relationship. A peer
 *   that found the offer over gossip has no way to obtain a key, so requiring
 *   one would make the offer a lie.
 * - `gated` — servable to callers holding an API key whose policy the operator
 *   pre-agreed, and deliberately *not* announced: the counterparties already
 *   know it is there, and gossiping it would advertise capacity to callers who
 *   cannot use it.
 * - `private` — never announced, never served off-node.
 */
export type ServeVisibility = "network" | "gated" | "private";
...
export class ProviderClient {
	...
	async participate(password: string): Promise<ParticipateResponse> {
		return await this.rpc.call<ParticipateResponse>("tenzro_participate", [
			password,
		]);
	}
	...
	async getDownloadProgress(modelId: string): Promise<DownloadProgress> {
		return await this.rpc.call<DownloadProgress>("tenzro_getDownloadProgress", [
			modelId,
		]);
	}
	...
	async chat(modelId: string, messages: ChatMessage[]): Promise<ChatResponse> {
		return await this.rpc.call<ChatResponse>("tenzro_chat", [
			modelId,
			messages,
		]);
	}
	...
	async getHardwareProfile(): Promise<HardwareProfile> {
		return await this.rpc.call<HardwareProfile>(
	...
	async listModelEndpoints(): Promise<ModelEndpoint[]> {
		return await this.rpc.call<ModelEndpoint[]>(
	...
	async getModelLoad(modelId: string): Promise<ModelLoadInfo | undefined> {
		...
		const ep = endpoints.find(
			(e) => e.model_id === modelId || e.model_name === modelId,
		);
		return ep?.load;
	}
	...
	async listProviders(providerType?: string): Promise<NetworkProvider[]> {
		...
		return await this.rpc.call<NetworkProvider[]>(
